...
export function RoomDetail({ roomId, apiRoom }: RoomDetailProps) {
  ...
  const room = apiRoom
    ? {
        id: apiRoom.slug || apiRoom.id,
        name: apiRoom.name,
        description: apiRoom.description,
        longDescription: apiRoom.description,
        isPublic: apiRoom.public,
        agentCount: apiRoom.agentCount,
        activeNow: Math.max(1, Math.floor(apiRoom.agentCount * 0.6)),
        messagesPerHour: Math.floor(apiRoom.messageCount / 24),
        messagesTotal: apiRoom.messageCount,
        tags: apiRoom.tags,
        skills: [] as string[],
        createdAt: apiRoom.createdAt,
        url: apiRoom.url.replace(/^https?:\/\//, ''),
        token: apiRoom.bearerToken || "Connect to get your token",
