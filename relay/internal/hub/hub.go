...
// channel (size 256) that Phase 3's SSE handler consumes for real-time
...
type RoomHub struct {
	...
	// events is a buffered channel consumed by the Phase 3 SSE fanout handler.
	...
	for {
		...
		case cmd := <-h.subscribe:
			...
			for name, ch := range subscribers {
				...
				default:
					...
					h.logger.Warn("subscriber channel full, dropping agent_joined event",
						"room", h.ID.String(), "target", name)
				}
			}
			// Emit to the Phase 3 SSE events channel (non-blocking).
			h.emitToEvents(evt)
			cmd.resp <- nil
		...
		case cmd := <-h.unsubscribe:
			...
			for _, remainingCh := range subscribers {
				...
				default:
					h.logger.Warn("subscriber channel full, dropping agent_left event",
						"room", h.ID.String())
				}
			}
			h.emitToEvents(evt)
...
// Events returns the read-only events channel. Phase 3 SSE handlers consume
